...
function pdfChart(chartoptions, filename) {
  return new Promise(async (resolve, reject) => {
    const timestamp = Date.now();
    const filename = `${makeid(7)}-${timestamp}`;
    ...
    const html = `<script src="https://code.highcharts.com/highcharts.js"></script>
...
function createHtml(Pollar, filename) {
  return new Promise((resolve, reject) => {
    let htmlpath = `${Files_path}/html/${filename}.html`;
    fs.writeFileSync(
      htmlpath,
